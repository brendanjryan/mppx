...
export const Constants = {
  ...
  components: {
    ...
    authority: '@authority',
    /** The lowercase HTTP request method. */
    method: '@method',
...
export async function sign(
  ...
  appendHeader(
    headers,
    AttestationHeaders.signatureInput,
    serializeDictionary(new Map([[input.label, toInnerList(input)]])),
  )
  appendHeader(
    headers,
    AttestationHeaders.signature,
    serializeDictionary(new Map([[input.label, [signature, new Map()]]])),
  )
...
export async function verify(
  ...
  if (
    options.nonceStore &&
    (await options.nonceStore.consume(`${input.keyId}:${input.nonce}`, input.expires * 1000))
  )
...
function parse(
  ...
  if (!inputHeader) return undefined
  const signatureHeader = request.headers.get(AttestationHeaders.signature)
  if (!signatureHeader) return { reason: 'No matching HTTP message signature was provided.' }

    ...
    async verify({ credential, request }) {
      ...
      switch (payload.type) {
        case 'hash': {
          ...
          const transfers = assertTransferCalls(calls, { currency, transfers: expectedTransfers })
        ...
        case 'transaction': {
          ...
          const expectedTransfers = getExpectedTransfers({ amount, memo, methodDetails, recipient })
          const transfers = assertTransferCalls(transaction.calls, {
            currency,
...
/** Branded type proving that calldata has been verified against expected transfers. */
type VerifiedTransfers = readonly ExpectedTransfer[] & { readonly __brand: 'VerifiedTransfers' }

function assertTransferCalls(
  calls: readonly { data?: `0x${string}` | undefined; to?: string | undefined }[],
  parameters: {
    currency: `0x${string}`
  ...
  return parameters.transfers as VerifiedTransfers
}
...
function getTransferCallsFromTransaction(transaction: {
  calls?: readonly { data?: `0x${string}` | undefined; to?: string | undefined }[] | undefined
...
function decodeTransferCall(
  call: { data?: `0x${string}` | undefined; to?: string | undefined },
  currency: `0x${string}`,
) {
  if (!call.to || !TempoAddress.isEqual(call.to as `0x${string}`, currency) || !call.data)
    return null
...
function assertTransferLogs(
  ...
  for (const transfer of parameters.transfers) {
    const matchIndex = logs.findIndex((log, index) => {
      ...
      if (!TempoAddress.isEqual(log.args.from, parameters.from as `0x${string}`)) return false
